...
{
	int num_element = width * height * channels;
	image_t* image;
	...
	{
		int size = sizeof(unsigned char) * num_element;
		image->ldr_buffer = (unsigned char*)malloc(size);
		memset(image->ldr_buffer, 0, size);
	}
	else
	{
		int size = sizeof(float) * num_element;
...
static void handle_button_message(window_t* window, button_t button, char pressed)
{
...
static LRESULT CALLBACK process_message(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)//这个回调函数没写好导致相机无法移动
{
...
static HWND create_window(const char* title_, int width, int height)
{
	...
	handle = CreateWindow(WINDOW_CLASS_NAME, title, style, CW_USEDEFAULT, CW_USEDEFAULT, width, height, NULL, NULL, GetModuleHandle(NULL), NULL);
...
window_t* window_create(const char* title, int width, int height, int text_width, int text_height)
{
	...
	text_handle = CreateWindow(TEXT("static"), TEXT("static"), WS_CHILD | WS_VISIBLE | WS_TABSTOP, 0, 0, text_width, text_height, handle, HMENU(21), GetModuleHandle(NULL), NULL);
