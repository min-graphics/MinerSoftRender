...
{
	float x = (ndc_coords.x + 1) * 0.5f * (float)width;
	float y = (ndc_coords.y + 1) * 0.5f * (float)height;
	float z = (ndc_coords.z + 1) * 0.5f;
...
Matrix4 lookat(Vector3f eye, Vector3f target, Vector3f up)
{
	...
	Vector3f y_axis = cross(z_axis, x_axis).normalize();
