...
static void interpolate_varying(shader_struct_v2f* v2f, shader_struct_v2f* ret, int sizeof_varying, Vector3f weights, float recip_w[3])
{
	int num_floats = sizeof_varying / sizeof(float);
...
//扫描线算法
/**/
void rasterize_triangle1(DrawData* draw_data, shader_struct_v2f* v2f)
{
	...
	for (int i = 0; i < 3; i++)
	{
		Vector3f win_coords = viewport_transform(render_buffer->width, render_buffer->height, ndc_coords[i]);
		screen_coords[i] = Vector2f(win_coords.x, win_coords.y);
		screen_depth[i] = win_coords.z;
	}
	...
	for (int i = 0; i < total_height; i++)
	{
		...
		//上半部分高度为最高顶点减去中间顶点的高度，下半部分为中间顶点的高度减去下面顶点的高度。
		...
		Vector2f B = second_half ? t1 + (t2 - t1) * beta : t0 + (t2 - t0) * beta;//求出当前y坐标对应的中间顶点与最低顶点连线上的点
		...
		for (int j = A.x; j < B.x; j++)
		{
			...
			interpolate_varying(v2f, &interpolate_v2f, sizeof(shader_struct_v2f), barycentric_weights, recip_w);
...
static void rasterize_triangle(DrawData* draw_data, shader_struct_v2f* v2f)
{
	...
	for (int i = 0; i < 3; i++)
	{
		ndc_coords[i] = proj<3>(v2f[i].clip_pos / v2f[i].clip_pos[3]);//这里一开始出错了
	}
